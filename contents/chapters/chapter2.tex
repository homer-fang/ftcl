...
Recent CPU/GPU heterogeneous systems in electronic-design automation provide useful comparisons for this thesis. GPU-accelerated static timing analysis and later CPU-GPU timing engines show that real systems must combine parallel kernels with scheduling, memory management, and industrial workflow constraints \cite{guo2020gpu,guo2023sta,guo2024heteroexcept,guo2026heterosta}. Although FTCL targets computational geometry instead of timing analysis, the systems-level lesson is similar: acceleration becomes convincing only when the runtime, memory abstraction, workload partitioning, and benchmark methodology are considered together.
